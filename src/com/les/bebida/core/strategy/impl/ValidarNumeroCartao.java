...
/**
 * Classe para validar o campo numero do Cartão de Credito
	...
	public String validar(EntidadeDominio entidade) {
		...
		if(cartao.getAlteraCartao().contentEquals("1")) {
			if(cartao.getNum_cartao() == null || cartao.getNum_cartao().equals("")) {
				return "Favor insira um numero no Cartão de Crédito.";
			}
			else if (cartao.getNum_cartao().length() < 16) {
				return "Favor insira um numero no Cartão de Crédito com no minimo 16 caracteres.";
			}
			else {
				return null;
			}
